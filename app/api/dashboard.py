...
def get_stats(db: Session = Depends(get_db)):
    ...
    stale = (
        db.query(func.count(WorkflowRun.id))
        .filter(WorkflowRun.status == "FAILED", WorkflowRun.updated_at < stale_cutoff)
        .scalar()
        or 0
    )

    # Count routing decisions from completed workflows
    decision_stages = (
        db.query(WorkflowStageState.output_json)
        .filter(
            WorkflowStageState.stage_name == "decision_routing",
            WorkflowStageState.status == "SUCCESS",
        )
        .all()
    )
...
def list_workflows_enhanced(
    ...
    if stale_only:
        query = query.filter(
            WorkflowRun.status == "FAILED", WorkflowRun.updated_at < stale_cutoff
        )
    ...
    failed_stages = (
        ...
        .filter(
            ...
            WorkflowStageState.status == "FAILED",
        )
